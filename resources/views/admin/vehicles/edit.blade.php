...
@section('title', 'Edit Vehicle - SriTravel Admin')
                ...
                <form action="{{ route('admin.vehicles.update', $vehicle->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div>
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="price_per_km" value="{{ old('price_per_km', $vehicle->price_per_km) }}" step="0.01"
                                           class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-blue-500 focus:border-blue-500">
                                    @error('price_per_km')
                                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="number" name="passengers" value="{{ old('passengers', $vehicle->passengers) }}" 
                                           class="w-full border border-gray-300 rounded-lg px-4 py-3 focus:ring-blue-500 focus:border-blue-500">
                                    @error('passengers')
                                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <input type="text" name="features" value="{{ old('features', implode(', ', $vehicle->features ?? [])) }}" 
                    ...
                    <div class="mt-8">
                        ...
                        <textarea name="description" rows="4" 
                                  class="w-full border border-gray-300 rounded-lg px-4 py-3">{{ old('description', $vehicle->description) }}</textarea>
                    </div>
                        ...
                        @if($vehicle->image)
                            <div class="mt-4">
                                ...
                                <img src="{{ asset('storage/'.$vehicle->image) }}" 
                                     class="max-w-xs rounded-lg border" alt="Current vehicle image">
                            </div>
                        @endif
                        @error('image')
                            <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
